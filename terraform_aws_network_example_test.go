...
func TestTerraformAwsNetworkExample(t *testing.T) {
	...
	vpcname := terraform.Output(t, terraformOptions, "main_vpc_name")
	vpcId := terraform.Output(t, terraformOptions, "main_vpc_id")
	vpccidr := terraformOptions.Output(t, terraformOptions, "main_vpc_cidr")
	...
	// Verify if the vpcname is correct
	assert.Equal(t, "terraform-network-example", vpcname)
	// Verify that the CIDR is correct
